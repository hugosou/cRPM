...
class FlexibleMultivariateNormal:
    ...
    def __init__(self,
                 ...
                 store_suff_stat_variance=False,
                 jitter=1e-6
    ):
        ...
        if init_natural:
            ...
            half_log_det_covariance = - torch.log(torch.abs(param_matrix.diagonal(dim1=-2, dim2=-1))).sum(-1) \
    ...
    def natural_parameters(self):
        ...
        T1, T2 = self.natural1, self.natural2
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        if (self.scale_tril is None) or (self.loc is None):
            raise ValueError("store_param_chol set to False")
...
def flexible_kl(p: FlexibleMultivariateNormal, q: FlexibleMultivariateNormal,
    ...
    if not (batch_shape_p == batch_shape_q):
        ...
        if (repeat1 is None) or (repeat2 is None):
            ...
            # New Batch Dim
            batch_len = 1 + max([max(repeat1, default=0), max(repeat2, default=0)])
...
def get_log_normalizer(mean, variance_diagonal, natural1):

    event_shape = mean.shape[-1]
    batch_shape = mean.shape[:-1]
    assert batch_shape == natural1.shape[:-1]
    assert event_shape == natural1.shape[-1]
    assert event_shape == variance_diagonal.shape[-1]
    assert batch_shape == variance_diagonal.shape[:-1]

    pi = torch.tensor([np.pi], dtype=mean.dtype, device=mean.device)
    half_log_det = 0.5 * torch.log(variance_diagonal).sum(dim=-1)
    half_mahalan = 0.5 * (mean * natural1).sum(dim=-1)
    half_offsets = 0.5 * event_shape * torch.log(2 * pi)

    return half_log_det + half_mahalan + half_offsets
